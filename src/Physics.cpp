...
void PhysicsBody::CreateCircle( int X, int Y, int R, int Segments) { //Create a circle primitive
	for (int I = 0; I < Segments; I++) {
		float angle = 2.0 * M_PI / Segments * I;
		new Vertex( this, X + R * cosf(angle), Y + R * sinf(angle));
	}
	
	for (int I = 0; I < VertexCount-1; I++) {
		for (int J = I + 1; J < VertexCount; J++) {
			new Edge( this, Vertices[ I ], Vertices[ J ], abs(I - J) == 1 || J - I == VertexCount - 1 );
		}
	}
}
